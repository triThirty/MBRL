...
class DynamicsEnsemble:
    ...
    def train(
        ...
        for epoch in range(epochs):
            for i, batch in enumerate(dataloader):
                ...
                loss_vals = list(
                    map(
                        lambda i: self.train_step(i, feed, target), range(self.n_models)
                    )
                )
                if summary_writer is not None:
                    for j, loss_val in enumerate(loss_vals):
                        summary_writer.add_scalar(
                            "Loss/dynamics_{}".format(j),
                            loss_val,
    ...
    def predict(self, x):
        ...
        with torch.set_grad_enabled(False):
            return torch.stack(
                list(map(lambda i: self.forward(i, x), range(self.n_models)))
            )
    ...
    def save(self, save_dir):
        for i in range(self.n_models):
            torch.save(
                ...
                os.path.join(save_dir, "dynamics_{}.pt".format(i)),
            )

    def load(self, load_dir):
        for i in range(self.n_models):
            self.models[i].load_state_dict(
                torch.load(os.path.join(load_dir, "dynamics_{}.pt".format(i)))
            )
