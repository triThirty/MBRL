...
class ActorCriticPolicy(nn.Module):
    ...
    def forward(self, obs, action=None):
        ...
        neg_log_prob = action_dist.log_prob(action) * -1.0
...
class ConvActorCriticPolicy(nn.Module):
    def __init__(
        ...
        self.var = torch.nn.Parameter(
            torch.tensor([0.0] * self.output_dim), requires_grad=True
        )
...
class PPO2:
    def __init__(
        self, input_dim, output_dim, device="cuda:0", network=ActorCriticPolicy
    ):
            ...
            for i in range(n_steps):
                if done:
                    ...
                    if not isinstance(env, FakeEnv):
                        obs = (
                            torch.from_numpy(obs[0]["observation"].copy())
                            .float()
                            .to(self.device)
                        )
                ...
                if isinstance(env, FakeEnv):
                    ...
                    obs, rewards, terminated, truncated, env_info = env.step(
                        action.cpu().numpy()
                    )
                    ...
                    # input()
                    obs = (
                        torch.from_numpy(obs["observation"].copy())
                        .float()
                        .to(self.device)
                    )
    ...
    def train_step(
        ...
        with torch.set_grad_enabled(True):
            ...
            actions, neg_log_probs, entropies, values = self.forward(
                obs, action=old_actions
            )

            loss, pg_loss, value_loss, entropy_mean, approx_kl = self.loss(
                clip_range,
                entropy_coef,
                value_coef,
                returns,
    ...
    def train(
        ...
        for update in tqdm(range(n_updates)):
            ...
            rewards, obs, returns, dones, actions, values, neg_log_probs, info = (
                self.generate_experience(env, n_steps, gamma, lam)
            )
            ...
            for i in range(num_train_epochs):
                ...
                for batch_num in range(num_batches):
                    ...
                    batch = (
                        arr[batch_indices]
                        for arr in (obs, returns, dones, actions, values, neg_log_probs)
                    )

                    # Run train step on batch
                    loss, pg_loss, value_loss, entropy, approx_kl = self.train_step(
                        clip_range, entropy_coef, value_coef, *batch
                    )
            ...
            if summary_writer is not None:
                ...
                summary_writer.add_scalar(
                    "Metrics/approx_kl", approx_kl, update * n_steps
                )
                summary_writer.add_scalar(
                    "Metrics/max_reward",
                ...
                summary_writer.add_scalar(
                    "Metricshalt_per_episode",
                    info["HALT"] / len(info["episode_rewards"]),
                    # step=update * n_steps,
                )
    ...
    def loss(
        ...
        values_clipped = old_values + torch.clamp(
            values - old_values, min=-clip_range, max=clip_range
        )
        ...
        pg_losses2 = -advantages * torch.clamp(
            ratios, 1.0 - clip_range, 1.0 + clip_range
        )
